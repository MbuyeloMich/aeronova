...
export class VehicleManager implements Updatable {
  ...
  private activeAircraftPresetId: string = AIRCRAFT_PRESETS[0].id;
  ...
  private waitForVehicleReady(vehicleId: string): void {
    ...
    const checkReady = () => {
      ...
      } else if (vehicle.hasLoadFailed()) {
        console.error(`Vehicle ${vehicleId} failed to load and will not be retried`);
      } else if (attempts >= maxAttempts) {
        console.error(`Timed out waiting for vehicle ${vehicleId} model readiness`);
      } else {
        attempts++;
  ...
  public async spawnAircraft(id: string = 'aircraft', position?: Cesium.Cartesian3, heading: number = 0): Promise<Vehicle> {
    const preset = this.getActiveAircraftPreset();
    const spawnPosition = position || Cesium.Cartesian3.fromDegrees(
      DEFAULT_SPAWN_LOCATION.lng,
      DEFAULT_SPAWN_LOCATION.lat,
      2300
    ...
    const aircraft = new Aircraft(id, {
      ...
      position: spawnPosition,
      heading,
      modelHeadingOffset: preset.modelHeadingOffset,
      physics: {
  ...
  public getAircraftPresets(): AircraftPreset[] {
    return AIRCRAFT_PRESETS;
  }

  public getActiveAircraftPreset(): AircraftPreset {
    return AIRCRAFT_PRESETS.find((preset) => preset.id === this.activeAircraftPresetId) ?? AIRCRAFT_PRESETS[0];
  }

  public getActiveAircraftPresetId(): string {
    return this.getActiveAircraftPreset().id;
  }

  public async setAircraftPreset(presetId: string): Promise<void> {
    const preset = AIRCRAFT_PRESETS.find((item) => item.id === presetId);
    if (!preset) return;

    this.activeAircraftPresetId = preset.id;
    const active = this.activeVehicle;
    if (!(active instanceof Aircraft)) return;
