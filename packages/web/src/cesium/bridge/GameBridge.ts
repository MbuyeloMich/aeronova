...
import type { AircraftPreset } from '../managers/VehicleManager';

export class GameBridge extends TypedEventEmitter<GameEvents> {
  private static readonly MIN_TELEPORT_CLEARANCE_METERS = 250;
  ...
  public getAircraftPresets(): AircraftPreset[] {
    return this.game.getVehicleManager().getAircraftPresets();
  }

  public getActiveAircraftPresetId(): string {
    return this.game.getVehicleManager().getActiveAircraftPresetId();
  }

  public setAircraftPreset(presetId: string): void {
    this.game.getVehicleManager().setAircraftPreset(presetId);
  }
  ...
  public teleportTo(longitude: number, latitude: number, altitude: number, heading: number = 0): void {
    const vehicle = this.game.getVehicleManager().getActiveVehicle();
    if (vehicle) {
      const requestedPosition = Cesium.Cartesian3.fromDegrees(longitude, latitude, altitude);
      const clampedGround = this.game.getScene().clampToHeight(requestedPosition);

      let safeAltitude = altitude;
      if (clampedGround) {
        const groundHeight = Cesium.Cartographic.fromCartesian(clampedGround).height;
        safeAltitude = Math.max(altitude, groundHeight + GameBridge.MIN_TELEPORT_CLEARANCE_METERS);
      }

      const newPosition = Cesium.Cartesian3.fromDegrees(longitude, latitude, safeAltitude);
  ...
  public restart(): void {
    ...
    if (vehicle && vehicle instanceof Aircraft && vehicle.isCrashed()) {
      ...
      const spawnPosition = Cesium.Cartesian3.fromDegrees(28.0484, -26.2342, 2300);
      const currentState = vehicle.getState();
      vehicle.setState({
        ...currentState,
